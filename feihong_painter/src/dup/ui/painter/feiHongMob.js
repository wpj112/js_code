...
***/
        ...
        this.bdom.bind(window, "scroll", function () {
            ...
            var adsToBottom = 0;
            if (window.fhShowStatus === 1){
                adsToBottom = Sheight - adsHeight;
            }
            if (((Sheight - (Stop + Cheight)) < (Cheight + adsToBottom)) && e.allowSend) {
                e.sendAdsRequest();
    ...
    renderFloatBanner:function (e){
        ...
        imgBack.id = 'imgBack_fixed';
        imgBack.src = 'http://10.128.136.21:8001/backbtn.jpg';
        imgBack.style.display = 'block';
        imgBack.style.position = 'absolute';
        imgBack.style.top = (screenWidth * 94 / 640) / 4;
        imgBack.style.height = (screenWidth * 94 / 640) / 3;
        imgBack.style.width = (screenWidth * 94 / 640) *2.7/ 3;
        imgBack.style.right = 10;
        bannerItem.appendChild(imgBack);
        e.fhBannerStatus =  1;
        e.bdom.bind(imgBack, 'click', e.onBackClick);
    },
